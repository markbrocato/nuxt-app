...
class DeployCommand {
  constructor(yargs) {
    _defineProperty(this, "_wrappedRun", async () => {
      var _this$_context$curren, _this$_context$curren2, _this$_context$curren3, _this$_context$curren4, _this$_context$curren5, _this$_context$curren6, _this$_context$curren7, _this$_context$curren8, _this$_context$curren9;
      ...
      const framework = await detectFramework(); // Remove this command in v8 EDGSITES-171

      if (siteParam) {
        deprecatedCommandWarning(this._context, 'site', 'property');
      }

      propertyParam = propertyParam || siteParam;
      ...
      const validator = new Validator(logger, {
        exitOnError: true
      }); // Get the token's site slug when site specific deploy token is used

      const deployTokenSiteSlug = (_this$_context$curren = (_this$_context$curren2 = this._context.currentActor) === null || _this$_context$curren2 === void 0 ? void 0 : (_this$_context$curren3 = _this$_context$curren2.deployToken) === null || _this$_context$curren3 === void 0 ? void 0 : (_this$_context$curren4 = _this$_context$curren3.site) === null || _this$_context$curren4 === void 0 ? void 0 : _this$_context$curren4.slug) !== null && _this$_context$curren !== void 0 ? _this$_context$curren : null;
      const deployTokenTeamSlug = (_this$_context$curren5 = (_this$_context$curren6 = this._context.currentActor) === null || _this$_context$curren6 === void 0 ? void 0 : (_this$_context$curren7 = _this$_context$curren6.deployToken) === null || _this$_context$curren7 === void 0 ? void 0 : (_this$_context$curren8 = _this$_context$curren7.site) === null || _this$_context$curren8 === void 0 ? void 0 : (_this$_context$curren9 = _this$_context$curren8.team) === null || _this$_context$curren9 === void 0 ? void 0 : _this$_context$curren9.slug) !== null && _this$_context$curren5 !== void 0 ? _this$_context$curren5 : null; // If --site is provided, override default $npm_package_name
      // If no site is provided, use other names in this order:

      const propertySlug = validator.validateSiteName(propertyParam || deployTokenSiteSlug || config.name || packageJson.name);
      ...
      teamSlug = teamSlug || deployTokenTeamSlug || config.team || (await this.checkIfSiteExistsOnAnotherTeam(propertySlug));
      await logger.title('📋 Deploying to:');
      logger.info([`> Team: ${teamSlug || 'Private space'}`, `> Property: ${propertySlug}`, branchName && `> branch: ${branchName}`, `> Environment: ${environment || 'default'}`, `> Edgio version: ${coreEdgioVersion}`, commitUrl && `> Commit URL: ${commitUrl}`].filter(Boolean) // Removes falsy values
      .join(EOL)); // In case we are in CI, setup additional alias to merge the events

      if (process.env.CI) {
        await this._context.analytics.setCIAlias(teamSlug, propertySlug);
      } // Throw an error when site's token doesn't belong to specified site via --site param.
      // Only warning from API is shown when site's name is retrieved from edgio.config.js


      if (propertyParam && deployTokenSiteSlug && propertyParam !== deployTokenSiteSlug) {
        logger.error(`Error: Site with name "${propertyParam}" can't be deployed with this token. The token allows access to site "${deployTokenSiteSlug}" only. ${EOL}Please specify the correct site name via --site param or use another token.`);
        process.exit(1);
      }
