...
class Context {
  constructor(argv) {
    ...
    if (command !== 'init') {// with edgio rebranding, we changed layer0.config.js name to edgio.config.js
  ...
  logout() {
    this.currentActor = null;
    saveApiKey(this.apiUrl, null); // Remove local analyticsID so we can create a new one on another login
