...
function transformRequestUrl(request) {
  ...
  let urlParam = url.searchParams.get('url') || url.searchParams.get('img');
  if (!urlParam) urlParam = url.pathname;
  const imgUrl = new URL(urlParam, `${protocol}localhost:${jsPort}`).toString();
